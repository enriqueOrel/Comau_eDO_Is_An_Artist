...
SIZE_BOARD = [150, 150]     # Size of the image. The image will be scaled keeping its aspect ratio
...
IMAGE_FILE = "Name_of_file.svg"           # Path of the SVG image, it can be relative to the current RDK station
...
def svg_draw_robot(svg_img, board, pix_ref, item_frame, item_tool, robot):
    ...
    APPROACH = 50  # approach distance in MM for each path    
    ...
    filename.writelines(str(home_joints))
    ...
    for path in svg_img:
        ...
        for i in range(np):
            ...
            robot.MoveL(target)
            filename.writelines(str(target))
...
if len(svgfile) == 0:
    ...
    svgfile = svgfile = path_stationfile + '/' + svgfile
...
board_draw.setVisible(False, False)
board_draw.setName('Board & image')
board_draw.Scale([size_img.x/150, size_img.y/150, 1]) # adjust the board size to the image size (scale)
